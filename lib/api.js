...
export const fetchCocktailsByIngredient = async (ingredient) => {
  ...
  export const fetchCocktailsByCategory = async (category) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/filter.php?c=${encodeURIComponent(category)}`);
      ...
      console.error('Error fetching cocktails by category:', error.message);
      return [];
    }
  };
