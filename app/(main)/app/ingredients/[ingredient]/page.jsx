...
import { fetchCocktailsByIngredient } from '@/lib/api';
...
const CocktailsByIngredient = () => {
  ...
  useEffect(() => {
    if (ingredient) {
      const loadCocktails = async () => {
        const cocktailsData = await fetchCocktailsByIngredient(ingredient);
  ...
  return (
    <div className="min-h-screen p-4">
      <h1 className="text-2xl font-bold mb-4">Cocktails with {ingredient}</h1>
      {isLoading ? (
